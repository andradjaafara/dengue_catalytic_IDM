...
data {
  ...
  int totalcases[nT]; // reported case data
...
parameters {
  ...
  real<lower=0,upper=0.15> lam_H[hist_length]; // historic average FOI
...
transformed parameters {
  ...
  real sum_lamH;
  vector<lower=0>[nT] Etotalcases;
  vector<lower=0,upper=1>[nT] Eproptotalcases;
  ...
  for(t in 1:nT) for(a in 1:nA){
    Ecases[t,a] = (mean(inc2[t,(ageLims[1,a]+1):(ageLims[2,a]+1)]) + gamma*mean(inc1[t,(ageLims[1,a]+1):(ageLims[2,a]+1)]))*pop[t,a];
  }
  ...
  for(t in 1:nT){
    Etotalcases[t] = sum(Ecases[t,]);
  }
  
  for(t in 1:nT){
    Eproptotalcases[t] = Etotalcases[t] ./ sum(Etotalcases);
  }
...
model {
  ...
  // for likelihood, need to add proportionality of reported cases over the reporting years
  // can treat them as multinomial
  // totalcases ~ multinomial(Eproptotalcases); // multinomial likelihood for total cases
}
